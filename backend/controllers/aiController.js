...
import { analyzeFeedback, analyzeFeedbackBatch } from '../services/nlp.js';

const MODEL = 'llama-3.3-70b-versatile';

export function getGroqClient(userApiKey) {
  const key = userApiKey || process.env.GROQ_API_KEY;
  if (!key) throw new Error('GROQ_API_KEY is not set');
  return new Groq({ apiKey: key });
}

async function generateText(prompt, userApiKey, temperature = 0.7) {
  const key = (userApiKey || process.env.GROQ_API_KEY || '').trim();
  if (!key) throw new Error('No Groq API key configured. Add GROQ_API_KEY to .env or set one in your profile.');
...
export async function generateFormFromPrompt(prompt, userApiKey) {
...
export async function suggestQuestions(existingQuestions, context, userApiKey) {
  const text = await generateText(
...
export async function analyzeResponses(formTitle, responses, userApiKey) {
  // Extract all text answers from all responses
  const allAnswers = responses.flatMap((r) =>
    (r.answers ?? []).filter((a) => typeof a.answer === 'string' && a.answer.trim())
  );
  ...
  if (allAnswers.length > 0) {
    const items = allAnswers.map((a) => ({ text: a.answer }));
    try {
      ...
      overallSentiment = Object.entries(counts).sort((a, b) => b[1] - a[1])[0][0];
...
export async function scoreResponseIntegrity(questions, answers, userApiKey) {
...
export async function analyzeSentiment(textResponses, userApiKey) {
  if (textResponses.length === 0) return 'neutral';

  // Use the trained model for sentiment (primary path)
  try {
    const items = textResponses.map((t) => ({ text: t }));
